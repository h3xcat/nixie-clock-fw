# 1 "c:\\dev\\nixie-clock-fw\\NixieClockFW\\NixieClockFW.ino"
# 2 "c:\\dev\\nixie-clock-fw\\NixieClockFW\\NixieClockFW.ino" 2
...
void gps_sync() {
  static uint32_t last_updated = 30*(60ul*1000);

  if (millis() - last_updated <= 30*(60ul*1000))
    return;
...
void display_update() {
  static uint32_t last_updated = 200*1ul;
  if (millis() - last_updated <= 200*1ul)
    return;
  last_updated = millis();
  ...
  if (Menu.buttonState(0) || Menu.buttonState(1) || Menu.buttonState(2)) {
...
void setup() {
  Serial.begin(230400);
  Serial.write("H3xCat's NixieClock Firmware (" "NCS318" ")\r\n");
